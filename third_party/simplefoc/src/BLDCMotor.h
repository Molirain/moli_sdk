...
class BLDCMotor : public FOCMotor {
  public:
    ...
    BLDCMotor(int pp, float R = NOT_SET, float KV = NOT_SET,
              float L_q = NOT_SET, float L_d = NOT_SET);
    ...
    /**
     * Measure resistance and inductance of a BLDCMotor and print results to
     * debug. If a sensor is available, an estimate of zero electric angle will
     * be reported too.
