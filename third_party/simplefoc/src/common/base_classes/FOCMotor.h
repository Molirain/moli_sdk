...
enum MotionControlType : uint8_t {
    ...
    angle_openloop = 0x04,
    angle_nocascade =
        0x05,     //!< Position/angle motion control without velocity cascade
...
enum FOCMotorStatus : uint8_t {
    ...
    motor_uncalibrated =
        0x02, //!< Motor is initialized, but not calibrated (open loop possible)
...
class FOCMotor {
  public:
     ...
     * If a sensor is available, an estimate of zero electric angle will be
     * reported too.
     * @param voltage The voltage applied to the motor
     * @param correction_factor  Is 1.5 for 3 phase motors, because we measure
     * over a series-parallel connection. TODO: what about 2 phase motors?
     * @returns 0 for success, >0 for failure
     */
    int characteriseMotor(float voltage, float correction_factor);

    /**
     * Auto-tune the current controller PID parameters based on desired
     * bandwidth. Uses a simple method that assumes a first order system and
     * requires knowledge of the motor phase resistance and inductance (if not
     * set, the characteriseMotor function can be used).
     *
    ...
    DQVoltage_s
        feed_forward_voltage; //!< current d and q voltage set to the motor
    ...
    float phase_inductance; //!< motor phase inductance q axis - FOR BACKWARDS
                            //!< COMPATIBILITY
    DQ_s axis_inductance{NOT_SET,
                         NOT_SET}; //!< motor direct axis phase inductance
    ...
    LowPassFilter LPF_velocity{
        DEF_VEL_FILTER_Tf}; //!<  parameter determining the velocity Low pass
                            //!<  filter configuration
    LowPassFilter LPF_angle{0.0}; //!<  parameter determining the angle low pass
                                  //!<  filter configuration
    unsigned int motion_downsample =
        DEF_MOTION_DOWNSMAPLE; //!< parameter defining the ratio of downsampling
                               //!< for move commad
    ...
    Direction sensor_direction =
        Direction::UNKNOWN; //!< default is CW. if sensor_direction ==
                            //!< Direction::CCW then direction will be flipped
                            //!< compared to CW. Set to UNKNOWN to set by
                            //!< calibration
    bool pp_check_result =
        false; //!< the result of the PP check, if run during loopFOC
     ...
     * - if torque control: target is set to 0V or 0A depending on torque
     * control type
     */
     ...
     * @note the custom control method has to be defined by the user and should
     * follow the signature: float controlMethod(FOCMotor* motor)
     * @param controlMethod - pointer to the custom control method function
     * defined by the user
     */
  ...
  protected:
     ...
     * @note - using _micros() function - be aware of its overflow every ~70
     * minutes
     */
    void updateLoopFOCTime() {
        updateTime(loopfoc_time_us, last_loopfoc_time_us,
    ...
    uint32_t last_loopfoc_timestamp_us =
        0; //!< timestamp of the last loopFOC execution in microseconds
    uint32_t last_loopfoc_time_us =
        0; //!< last elapsed time of loopFOC in microseconds
    uint32_t last_move_timestamp_us =
        0; //!< timestamp of the last move execution in microseconds
    uint32_t last_move_time_us =
        0; //!< last elapsed time of move in microseconds
  private:
    ...
    // It filters the value using low pass filtering alpha = 0.1
    void updateTime(uint32_t &elapsed_time_filetered, uint32_t &elapsed_time,
                    uint32_t &last_timestamp_us, float alpha = 0.1f) {
        uint32_t now = _micros();
        elapsed_time = now - last_timestamp_us;
        elapsed_time_filetered =
            (1 - alpha) * elapsed_time_filetered + alpha * elapsed_time;
