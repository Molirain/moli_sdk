...
FOCMotor::FOCMotor() {
    ...
    // Initialize phase voltages U alpha and U beta used for inverse Park and
    // Clarke transform
    Ualpha = 0;
    Ubeta = 0;
...
int FOCMotor::characteriseMotor(float voltage, float correction_factor = 1.0f) {
    ...
    DQCurrent_s zerocurrent = current_sense->getDQCurrents(
        current_sense->getABCurrents(zerocurrent_raw), current_electric_angle);

    // Ramp and hold the voltage to measure resistance
    // 300 ms of ramping
    current_electric_angle = electricalAngle();
    for (int i = 0; i < 100; i++) {
        setPhaseVoltage(0, voltage / 100.0 * ((float)i),
                        current_electric_angle);
        _delay(3);
    }
    _delay(10);
    PhaseCurrent_s r_currents_raw = current_sense->readAverageCurrents();
    DQCurrent_s r_currents = current_sense->getDQCurrents(
        current_sense->getABCurrents(r_currents_raw), current_electric_angle);
    ...
    float resistance =
        voltage / (correction_factor * (r_currents.d - zerocurrent.d));
    if (resistance <= 0.0f) {
    ...
    unsigned int risetime_us =
        200; // initially short for worst case scenario with low inductance
    unsigned int settle_us =
        100000; // initially long for worst case scenario with high inductance

    // Pre-rotate the angle to the q-axis (only useful with sensor, else no harm
    // in doing it)
    ...
    for (size_t axis = 0; axis < 2; axis++) {
        for (size_t i = 0; i < iterations; i++) {
            // current_electric_angle = i * _2PI / iterations; // <-- Do a sweep
            // of the inductance. Use eg. for graphing
            ...
            for (size_t j = 0; j < cycles; j++) {
                ...
                zerocurrent = current_sense->getDQCurrents(
                    current_sense->getABCurrents(zerocurrent_raw),
                    current_electric_angle);

                // step the voltage
                setPhaseVoltage(0, voltage, current_electric_angle);
                t0 = _micros();
                _delayMicroseconds(risetime_us); // wait a little bit

                PhaseCurrent_s l_currents_raw =
                    current_sense->getPhaseCurrents();
                t1 = _micros();
                setPhaseVoltage(0, 0, current_electric_angle);

                DQCurrent_s l_currents = current_sense->getDQCurrents(
                    current_sense->getABCurrents(l_currents_raw),
                    current_electric_angle);
                _delayMicroseconds(
                    settle_us); // wait a bit for the currents to go to 0 again
                ...
                if (l_currents.d - zerocurrent.d <= 0 ||
                    (voltage - resistance * (l_currents.d - zerocurrent.d)) <=
                        0) {
                    continue;
                }
                ...
                inductanced +=
                    fabsf(-(resistance * dt) /
                          log((voltage -
                               resistance * (l_currents.d - zerocurrent.d)) /
                              voltage)) /
                    correction_factor;

                qcurrent += l_currents.q -
                            zerocurrent.q; // average the measured currents
            ...
            float timeconstant = fabsf(
                Ltemp / resistance); // Timeconstant of an RL circuit (L/R)
            // SIMPLEFOC_MOTOR_DEBUG("Estimated time constant in us: ",
            // 1000000.0f * timeconstant);

            // Wait as long as possible (due to limited timing accuracy & sample
            // rate), but as short as needed (while the current still changes)
            risetime_us = _constrain(risetime_us * 0.6f +
                                         0.4f * 1000000 * 0.6f * timeconstant,
                                     100, 10000);
            settle_us = 10 * risetime_us;

            // Serial.printf(">inductance:%f:%f|xy\n", current_electric_angle,
            // Ltemp * 1000.0f); // <-- Plot an angle sweep

            /**
             * How this code works:
             * If we apply a current spike in the d´-axis, there will be cross
             * coupling to the q´-axis current, if we didn´t use the actual
             * d-axis (ie. d´ != d). This has to do with saliency (Ld != Lq).
             * The amount of cross coupled current is somewhat proportional to
             * the angle error, which means that if we iteratively change the
             * angle to min/maximise this current, we get the correct d-axis
             * (and q-axis).
             */
            if (axis) {
            ...
            // Average the d-axis angle further for calculating the electrical
            // zero later
            if (axis) {
                d_electrical_angle = i < 2 ? current_electric_angle
                                           : d_electrical_angle * 0.9 +
                                                 current_electric_angle * 0.1;
            }
        }

        // We know the minimum is 0.5*PI radians away, so less iterations are
        // needed.
        current_electric_angle += 0.5f * _PI;
        current_electric_angle = _normalizeAngle(current_electric_angle);
        iterations /= 2;
    ...
    if (sensor) {
        /**
         * The d_electrical_angle should now be aligned to the d axis or the -d
         * axis. We can therefore calculate two possible electrical zero angles.
         * We then report the one closest to the actual value. This could be
         * useful if the zero search method is not reliable enough (eg. high
         * pole count).
         */

        float estimated_zero_electric_angle_A =
            _normalizeAngle((float)(sensor_direction * pole_pairs) *
                                sensor->getMechanicalAngle() -
                            d_electrical_angle);
        float estimated_zero_electric_angle_B =
            _normalizeAngle((float)(sensor_direction * pole_pairs) *
                                sensor->getMechanicalAngle() -
                            d_electrical_angle + _PI);
        float estimated_zero_electric_angle = 0.0f;
        if (fabsf(estimated_zero_electric_angle_A - zero_electric_angle) <
            fabsf(estimated_zero_electric_angle_B - zero_electric_angle)) {
...
void FOCMotor::monitor() {
    ...
    // read currents if possible - even in voltage mode (if current_sense
    // available)
...
float FOCMotor::angleOpenloop(float target_angle) {
    ...
    // target angle with maximal velocity (velocity_limit)
    ...
    //                        captured by the precision of floats when the total
    //                        position is large.
    if (abs(target_angle - shaft_angle) > abs(velocity_limit * Ts)) {
        shaft_angle +=
            _sign(target_angle - shaft_angle) * abs(velocity_limit) * Ts;
...
void FOCMotor::updateVelocityLimit(float new_velocity_limit) {
    ...
    if (controller != MotionControlType::angle_nocascade)
        P_angle.limit = abs(
            velocity_limit); // if angle control but no velocity cascade, limit
                             // the angle controller by the velocity limit
}
...
void FOCMotor::updateCurrentLimit(float new_current_limit) {
    ...
    if (torque_controller != TorqueControlType::voltage) {
        ...
        if (controller == MotionControlType::angle_nocascade)
            // if angle control but no velocity cascade, limit the angle
            // controller by the current limit
...
void FOCMotor::updateVoltageLimit(float new_voltage_limit) {
    ...
    if (torque_controller == TorqueControlType::voltage) {
        ...
        if (controller == MotionControlType::angle_nocascade)
            // if angle control but no velocity cascade, limit the angle
            // controller by the voltage limit
            P_angle.limit = new_voltage_limit;
    }
}

// Update torque control type and related controller limit values
void FOCMotor::updateTorqueControlType(
    TorqueControlType new_torque_controller) {
...
void FOCMotor::updateMotionControlType(
    MotionControlType new_motion_controller) {

    if (controller == new_motion_controller)
        return; // no change
    ...
    case MotionControlType::angle:
        if (controller == MotionControlType::angle_openloop ||
            controller == MotionControlType::angle_nocascade)
            break;
...
int FOCMotor::tuneCurrentController(float bandwidth) {
    ...
    if (loopfoc_time_us && bandwidth > 0.5f * (1e6f / loopfoc_time_us)) {
        ...
        SIMPLEFOC_MOTOR_ERROR("Fail. BW too high, current loop freq:",
                              (1e6f / loopfoc_time_us));
        return 2;
    }
    if (!_isset(phase_resistance) ||
        (!_isset(phase_inductance) && !_isset(axis_inductance.q))) {
    ...
    LPF_current_d.Tf =
        1.0f / (_2PI * bandwidth * 5.0f); // filter cutoff at 5x bandwidth
    LPF_current_q.Tf =
        1.0f / (_2PI * bandwidth * 5.0f); // filter cutoff at 5x bandwidth
...
void FOCMotor::loopFOC() {
    ...
    if (motor_status == FOCMotorStatus::motor_calibrating)
        return;
    ...
    case TorqueControlType::voltage:
        voltage.q = _constrain(current_sp, -voltage_limit, voltage_limit) +
                    feed_forward_voltage.q;
    ...
    case TorqueControlType::estimated_current:
        ...
        current_sp = _constrain(current_sp, -current_limit, current_limit) +
                     feed_forward_current.q; // desired current is the setpoint
        // calculate the back-emf voltage if KV_rating available U_bemf =
        // vel*(1/KV)
        if (_isset(KV_rating))
            voltage_bemf = estimateBEMF(shaft_velocity);
        ...
        if (_isset(axis_inductance.q))
            voltage.d = _constrain(-current_sp * shaft_velocity * pole_pairs *
                                       axis_inductance.q,
                                   -voltage_limit, voltage_limit) +
                        feed_forward_voltage.d;
        else
            voltage.d = feed_forward_voltage.d;
        break;
    case TorqueControlType::dc_current:
        ...
        // calculate the phase voltage
        voltage.q =
            PID_current_q(current_sp - current.q) + feed_forward_voltage.q;
        // d voltage  - lag compensation
        if (_isset(axis_inductance.q))
            voltage.d = _constrain(-current_sp * shaft_velocity * pole_pairs *
                                       axis_inductance.q,
    ...
    case TorqueControlType::foc_current:
        ...
        if (_isset(axis_inductance.q))
            voltage.d =
                _constrain(voltage.d - current_sp * shaft_velocity *
                                           pole_pairs * axis_inductance.q,
                           -voltage_limit, voltage_limit);
        // q voltage - cross coupling compensation - TODO verify
        if (_isset(axis_inductance.d))
            voltage.q =
                _constrain(voltage.q + current.d * shaft_velocity * pole_pairs *
                                           axis_inductance.d,
                           -voltage_limit, voltage_limit);
        // add feed forward
...
void FOCMotor::move(float new_target) {
    ...
    // except for the open loop modes where the values are updated within
    // angle/velocityOpenLoop functions

    // shaft angle/velocity need the update() to be called first
    // get shaft angle
    // TODO sensor precision: the shaft_angle actually stores the complete
    // position, including full rotations, as a float
    //                        For this reason it is NOT precise when the angles
    ...
    if (controller != MotionControlType::angle_openloop &&
        ...
        // because in open loop the shaft angle/velocity is updated within
        // angle/velocityOpenLoop functions
    ...
    case MotionControlType::torque:
        ...
        break;
    case MotionControlType::angle_nocascade:
        // TODO sensor precision: this calculation is not numerically precise.
        // The target value cannot express precise positions when
        //                        the angles are large. This results in not
        //                        being able to command small changes at high
        //                        position values. to solve this, the
        //                        delta-angle has to be calculated in a
        //                        numerically precise way.
        // angle set point
        shaft_angle_sp = target;
        // calculate the torque command - sensor precision: this calculation is
        // ok, but based on bad value from previous calculation
    ...
    case MotionControlType::angle:
        ...
        shaft_velocity_sp =
            _constrain(shaft_velocity_sp, -velocity_limit, velocity_limit);
        // calculate the torque command - sensor precision: this calculation is
        // ok, but based on bad value from previous calculation
        current_sp = PID_velocity(shaft_velocity_sp - shaft_velocity);
        break;
    case MotionControlType::velocity:
        // velocity set point - sensor precision: this calculation is
        // numerically precise.
        ...
        break;
    case MotionControlType::velocity_openloop:
        // velocity control in open loop - sensor precision: this calculation is
        // numerically precise.
        shaft_velocity_sp = target;
        // this function updates the shaft_angle and shaft_velocity
        // returns the voltage or current that is to be set to the motor
        // (depending on torque control mode) returned values correspond to the
        // voltage_limit and current_limit
    ...
    case MotionControlType::angle_openloop:
        ...
        // subject
        //                        to the same problems in small set-point
    ...
    case MotionControlType::custom:
        // custom control - user provides the function that calculates the
        // current_sp based on the target value and the motor state user makes
        // sure to use it with appropriate torque control mode
...
int FOCMotor::alignCurrentSense() {
    ...
    exit_flag =
        current_sense->driverAlign(voltage_sensor_align, modulation_centered);
    if (!exit_flag) {
...
int FOCMotor::alignSensor() {
    ...
    if (sensor_direction == Direction::UNKNOWN) {
        ...
        if (moved <
            MIN_ANGLE_DETECT_MOVEMENT) { // minimum angle to detect movement
...
int FOCMotor::absoluteZeroSearch() {
    // sensor precision: this is all ok, as the search happens near the 0-angle,
    // where the precision
