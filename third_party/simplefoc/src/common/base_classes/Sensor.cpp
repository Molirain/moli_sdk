...
float Sensor::getVelocity() {
    ...
    // if timestamps were unsigned, we could get rid of this section, unsigned
    // overflow handles it correctly
    float Ts = (angle_prev_ts - vel_angle_prev_ts) * 1e-6f;
    if (Ts <
        0.0f) { // handle micros() overflow - we need to reset vel_angle_prev_ts
    ...
    // floating point equality checks are bad, so instead we check that the
    // angle change is very small
